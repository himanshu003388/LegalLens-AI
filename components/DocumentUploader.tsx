...
import { UploadCloud, FileText, Shield, CheckCircle2, Sparkles, AlertCircle, Key } from "lucide-react";
...
export default function DocumentUploader({
  ...
  const [engineLabel, setEngineLabel] = useState("Local Engine (Zero-Key)");
  ...
  useEffect(() => {
    const updateLabel = () => {
      ...
      if (!key) {
        setEngineLabel("Local Engine (Add Gemini Key)");
      } else if (prov === "gemini" || key.startsWith("AIza")) {
        setEngineLabel("Active: Google Gemini 1.5");
      } else if (prov === "openai") {
        setEngineLabel("Active: OpenAI GPT-4o");
      } else if (prov === "anthropic") {
        setEngineLabel("Active: Claude 3.5 Sonnet");
      } else {
        setEngineLabel("Active: Cloud AI Key");
      }
    };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/90 hover:bg-slate-100 dark:hover:bg-slate-700/80 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 transition shadow-sm"
